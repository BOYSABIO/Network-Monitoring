...
The ML pipeline in `src/` started as a focused **network intrusion detection** project: classify network flows as benign vs malicious using realistic features (Zeek/Argus-style flow data).

**Goal:** Build interpretable baselines and progress to stronger models for flow-level binary classification.

**Why it's employable:** Mirrors SOC workflows for triage, supports explainability for analyst trust, and demonstrates ingestion from PCAP → features → serving.

**Scope outline:**
- **Data:** PCAP → Zeek → features; public CSV subset for reproducibility
- **Baselines:** Logistic regression and tree-based models; compare AUC/PR; calibration
...
Experiment tracking and model artifacts are supported via [Weights & Biases](https://wandb.ai/).

1. Install dependencies (includes `wandb` in `requirements.txt`).
2. Authenticate: `wandb login` or `export WANDB_API_KEY=...`
3. Enable in `src/config/config.yaml`:
...
```
config         Configuration loader (config.yaml) — paths, feature lists, model settings
   ↓
data_load      Load raw CSV data
   ↓
data_validation   Schema checks and data quality gates
   ↓
preprocess     Clean, encode categoricals, drop columns
   ↓
model          Train/test split, scaling, GridSearchCV, save artifact + enriched splits
   ↓
evaluation     Metrics, confusion matrix, ROC curves (uses saved test split)
   ↓
inference      Classify new connections using a trained model
   ↓
ingestion      PCAP → Zeek → feature DataFrame (batch and live; feeds inference)
   ↓
api            REST API for serving predictions
```

Feature column definitions (`numeric`, `categorical`, etc.) live in `config.yaml` and are used by preprocessing, training, and inference.
...
- Multi-class attack family classification
- Near-real-time inference demo
